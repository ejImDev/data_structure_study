...
public class UnionFind {
    ...
    public UnionFind(int N){
        ...
        rank = new int[N];
        for (int i=0; i<N; i++){
            p[i] = i; // 초기엔 N개의 트리가 각각 i 자기 자신이 부모이기 때문에
    ...
    protected int find(int i) { // 경로 압축
        if(i!=p[i]){
            p[i] = find(p[i]); // 리턴하며 경로상의 각 노드가 부모가 루트가 되도록 만든다
        }
        return p[i];
    }
    // i와 j가 같은 트리에 있는지를 검사한다
